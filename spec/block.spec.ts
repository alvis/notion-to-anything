...
describe('fn:getBlocks', () => {
  ...
  it('should not descend into child_page blocks even when has_children is true', async () => {
    ...
    const listSpy = vi.spyOn(client.blocks.children, 'list').mockResolvedValue({
    ...
    // only the parent listing is fetched — the child page is not descended into
    expect(listSpy).toHaveBeenCalledTimes(1);
    expect(result).toEqual([
      { ...childPageBlock, has_children: true, children: [] },
    ]);
  });

  it('should not descend into child_database blocks even when has_children is true', async () => {
    const childDatabaseBlock = {
      ...defaultBlockProperties,
      id: 'child-database-block',
      type: 'child_database',
      child_database: { title: 'Nested Database' },
      has_children: true,
    } satisfies NotionAPIBlock;
    const listSpy = vi.spyOn(client.blocks.children, 'list').mockResolvedValue({
      results: [childDatabaseBlock],
      has_more: false,
      next_cursor: null,
      object: 'list',
      type: 'block',
    ...
    // only the parent listing is fetched — the child database is not descended into
    expect(listSpy).toHaveBeenCalledTimes(1);
    expect(result).toEqual([
      { ...childDatabaseBlock, has_children: true, children: [] },
    ]);
  });

  it('should still descend into normal blocks with children alongside non-descended child pages', async () => {
    const childPageBlock = {
      ...defaultBlockProperties,
      id: 'child-page-block',
      type: 'child_page',
    ...
    const toggleBlock = {
      ...
      id: 'toggle-block',
      type: 'toggle',
      toggle: { rich_text: [], color: defaultColor },
      has_children: true,
    } satisfies NotionAPIBlock;
    const grandChild = {
      ...defaultBlockProperties,
      id: 'toggle-child',
      type: 'paragraph',
      paragraph: { rich_text: [], color: defaultColor, icon: null },
      has_children: false,
    } satisfies NotionAPIBlock;
    const listSpy = vi.spyOn(client.blocks.children, 'list');
    listSpy
      .mockResolvedValueOnce({
        results: [childPageBlock, toggleBlock],
        has_more: false,
        next_cursor: null,
        object: 'list',
        type: 'block',
    ...
    // exactly two listings: the parent and the toggle's children;
    // the child page is never descended into
    ...
    expect(result).toEqual([
      { ...childPageBlock, has_children: true, children: [] },
      { ...toggleBlock, has_children: true, children: [grandChild] },
    ]);
  });
  ...
  it('should throw a NotionAnythingError named AbortError when signal is already aborted', async () => {
    const controller = new AbortController();
    controller.abort();

    const error = await getBlocks(client, 'any-id', {
  ...
  it('should surface a NotionAPIError when the block listing raises a Notion SDK error', async () => {
    ...
    vi.spyOn(client.blocks.children, 'list').mockRejectedValue(sdkError);

    const error = await getBlocks(client, 'parent-id').catch(
