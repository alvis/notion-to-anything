...
import { enrichEntity, NotionEntity } from '#entity';
...
import { personUser } from './fixtures/users';
...
vi.mock(
  '#metadata',
  () => ({ getMetadata }) satisfies Partial<typeof import('#metadata')>,
);
...
describe('cl:NotionEntity', () => {
  ...
  describe('op:getParent', () => {
    ...
    it('should throw for inaccessible page parent', async () => {
      ...
      const error = await entity.getParent().catch((reason: unknown) => reason);

      expect(error).toBeInstanceOf(NotionAnythingError);
      expect((error as NotionAnythingError).code).toBe('ENTITY_NOT_ACCESSIBLE');
      expect((error as NotionAnythingError).message).toBe(
    ...
    it('should wrap a Notion SDK error into a NotionAPIError for a page parent', async () => {
      const fetch = vi.fn<typeof globalThis.fetch>();
      const testClient = new Client({
        fetch,
        logger: () => undefined,
      ...
      const childPage = buildDummyPage({
        pageID: 'child-page-id',
        parent: { type: 'page_id', page_id: 'missing-page-id' },
      });
      ...
      expect((error as NotionAPIError).code).toBe('object_not_found');
      expect((error as NotionAPIError).cause).toBeInstanceOf(APIResponseError);
    });

    it('should wrap a Notion SDK error into a NotionAPIError for a database parent', async () => {
      const fetch = vi.fn<typeof globalThis.fetch>();
      const testClient = new Client({
        fetch,
        logger: () => undefined,
      ...
      const childPage = buildDummyPage({
        pageID: 'child-page-id',
        parent: { type: 'database_id', database_id: 'missing-db-id' },
      });
    ...
    it('should wrap a Notion SDK error into a NotionAPIError for a datasource parent', async () => {
      const fetch = vi.fn<typeof globalThis.fetch>();
      const testClient = new Client({
        fetch,
        logger: () => undefined,
    ...
    it('should rethrow a non-SDK error unchanged from a page parent retrieval', async () => {
      const nonSdkError = new RangeError('boom');
      const fetch = vi.fn<typeof globalThis.fetch>(() => {
        throw nonSdkError;
      });
      const testClient = new Client({
      ...
      const childPage = buildDummyPage({
        pageID: 'child-page-id',
        parent: { type: 'page_id', page_id: 'boom-page-id' },
      });
      ...
      expect(error).toBe(nonSdkError);
    });

    it('should rethrow a non-SDK error unchanged from a database parent retrieval', async () => {
      const nonSdkError = new RangeError('boom');
      const fetch = vi.fn<typeof globalThis.fetch>(() => {
        throw nonSdkError;
      });
      ...
      const childPage = buildDummyPage({
        pageID: 'child-page-id',
        parent: { type: 'database_id', database_id: 'boom-db-id' },
      });
    ...
    it('should rethrow a non-SDK error unchanged from a datasource parent retrieval', async () => {
      const nonSdkError = new RangeError('boom');
      const fetch = vi.fn<typeof globalThis.fetch>(() => {
        throw nonSdkError;
      });
...
describe('fn:enrichEntity', () => {
  ...
  it('should resolve partial created_by and last_edited_by', async () => {
    ...
    const result = await enrichEntity(page, resolver);
  ...
  it('should enrich datasource entities', async () => {
    ...
    const result = await enrichEntity(dataSource, resolver);
