...
describe('fn:getMetadata', () => {
  ...
  it.each([
    ...
    [
      'embedded',
      { type: 'file', file: { url: 'url', expiry_time: defaultTime } },
      'url',
    ],
  ...
  it.each([
    ...
    'should handle %s icon',
    (_type, icon, expectedEmoji, expectedImage) => {
  ...
  it.each([
    [
      'created_by (inaccessible)',
      'created_by',
      inaccessibleUser,
      { createdByAvatar: null, createdByEmail: null, createdByName: null },
    ],
    [
      'created_by (accessible)',
      'created_by',
      personUser,
      {
        createdByAvatar: 'url',
    ...
    const testDataSource = buildDummyDataSource({
      ...
      properties: {
        'Database Title': {
          ...
          name: 'Database Title',
          title: {},
          description: 'Main title property',
        },
      },
    });
    const expected = {
      ...
      createdAt: defaultTime,
      lastEditedAt: defaultTime,
      url: 'https://www.notion.so/workspace/test-database-id',
      publicUrl: null,
      inTrash: false,
      coverImage: 'https://www.notion.so/cover.png',
      createdByAvatar: 'url',
  ...
  it('should find title property from datasource properties when title field is empty', () => {
    const dataSourceWithPropertyTitle = {
      ...buildDummyDataSource({ id: 'property-title-db' }),
      title: [],
      properties: {
        'Database Name': {
          id: 'title-prop-id',
          ...
          name: 'Database Name',
          title: {},
          description: 'Title property in properties',
        },
      },
    };
    const expected = {
      id: 'property-title-db',
      title: '',
      createdAt: defaultTime,
      lastEditedAt: defaultTime,
      url: 'https://www.notion.so/workspace/property-title-db',
      publicUrl: null,
      inTrash: false,
      coverImage: 'https://www.notion.so/cover.png',
      createdByAvatar: 'url',
  ...
  it('should handle metadata extraction with null values in datasource', () => {
    const minimalDataSource = {
      ...
      created_by: null,
      last_edited_by: null,
    } as unknown as NotionAPIDataSource;
    const expected = {
      id: 'database-id',
      title: 'Title',
      createdAt: defaultTime,
  ...
  it('should handle database with description', () => {
    const dbWithDescription = buildDummyDatabase({
      id: 'described-db',
