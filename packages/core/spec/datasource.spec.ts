...
import * as takeModule from '#traversal/take';
...
import { setupDataSource } from '../../../fixtures/mocks/api';
...
describe('cl:NotionDataSource', () => {
  ...
  describe('op:search', () => {
    ...
    it('should emulate offset by over-fetching and slicing', async () => {
      ...
      const takeSpy = vi.spyOn(takeModule, 'take').mockResolvedValue({
        entities: [
          buildDummyPage({ pageID: 'page0', parent: { type: 'database_id', database_id: 'offset-db' } }),
          buildDummyPage({ pageID: 'page1', parent: { type: 'database_id', database_id: 'offset-db' } }),
          buildDummyPage({ pageID: 'page2', parent: { type: 'database_id', database_id: 'offset-db' } }),
          buildDummyPage({ pageID: 'page3', parent: { type: 'database_id', database_id: 'offset-db' } }),
          buildDummyPage({ pageID: 'page4', parent: { type: 'database_id', database_id: 'offset-db' } }),
        ],
      });
      ...
      expect(takeSpy).toHaveBeenCalledWith(
        client.dataSources.query,
        { data_source_id: 'offset-db', filter: undefined, sorts: undefined },
    ...
    it('should ignore offset when cursor is provided', async () => {
      ...
      const takeSpy = vi.spyOn(takeModule, 'take').mockResolvedValue({
        entities: [
          buildDummyPage({ pageID: 'page0', parent: { type: 'database_id', database_id: 'cursor-offset-db' } }),
          buildDummyPage({ pageID: 'page1', parent: { type: 'database_id', database_id: 'cursor-offset-db' } }),
        ],
        next: 'next',
      });
      ...
      expect(takeSpy).toHaveBeenCalledWith(
        client.dataSources.query,
        { data_source_id: 'cursor-offset-db', filter: undefined, sorts: undefined },
  ...
  describe('gt:properties', () => {
    it('should return datasource properties', () => {
      const expected = {
        Status: { id: 'status-id', name: 'Status', type: 'select', description: null, select: { options: [{ id: '1', name: 'To Do', color: 'red', description: null }, { id: '2', name: 'Done', color: 'green', description: null }] } },
        Priority: { id: 'priority-id', name: 'Priority', type: 'number', description: null, number: { format: 'number' } },
