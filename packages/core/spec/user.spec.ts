...
describe('cl:UserResolver', () => {
  ...
  describe('mt:resolve', () => {
    ...
    it('should return null when API call fails', async () => {
      const errorResponse = { code: 'object_not_found', message: 'Could not find user.', object: 'error', request_id: 'request-id', status: 404 };
      fetch.mockResolvedValueOnce(
        new Response(JSON.stringify(errorResponse), { status: 404, headers: { 'Content-Type': 'application/json' } }),
      );
      const resolver = new UserResolver(client);
    ...
    it('should make only one API call for repeated resolution of same user ID', async () => {
      fetch.mockResolvedValueOnce(
        new Response(JSON.stringify(userResponse), { status: 200, headers: { 'Content-Type': 'application/json' } }),
      );
      const resolver = new UserResolver(client);
    ...
    it('should make separate calls for different user IDs', async () => {
      ...
      fetch
        .mockResolvedValueOnce(new Response(JSON.stringify(userResponse), { status: 200, headers: { 'Content-Type': 'application/json' } }))
        .mockResolvedValueOnce(new Response(JSON.stringify(user2), { status: 200, headers: { 'Content-Type': 'application/json' } }));
      const resolver = new UserResolver(client);
  ...
  describe('gt:size', () => {
    it('should return number of cached entries', async () => {
      fetch
        .mockResolvedValueOnce(new Response(JSON.stringify(userResponse), { status: 200, headers: { 'Content-Type': 'application/json' } }))
        .mockResolvedValueOnce(new Response(JSON.stringify(buildUser({ id: 'user-2' })), { status: 200, headers: { 'Content-Type': 'application/json' } }));
      const resolver = new UserResolver(client);
  ...
  describe('cache disabled', () => {
    ...
    it('should return zero for size when cache is disabled', async () => {
      fetch.mockResolvedValueOnce(new Response(JSON.stringify(userResponse), { status: 200, headers: { 'Content-Type': 'application/json' } }));
      const resolver = new UserResolver(client, { cache: false });
...
describe('fn:resolveEntityUsers', () => {
  ...
  it('should resolve partial created_by and last_edited_by', async () => {
    ...
    fetch.mockResolvedValue(new Response(JSON.stringify(fullUser), { status: 200, headers: { 'Content-Type': 'application/json' } }));
    const resolver = new UserResolver(client);
    const page = { ...buildDummyPage(), created_by: { object: 'user' as const, id: 'partial-user-id' }, last_edited_by: { object: 'user' as const, id: 'partial-user-id' } };
  ...
  it('should keep original partial user when resolution fails', async () => {
    const errorResponse = { code: 'object_not_found', message: 'Not found', object: 'error', request_id: 'req-id', status: 404 };
    fetch.mockResolvedValueOnce(new Response(JSON.stringify(errorResponse), { status: 404, headers: { 'Content-Type': 'application/json' } }));
    const resolver = new UserResolver(client);
    const partial = { object: 'user' as const, id: 'gone-user' };
    const page = { ...buildDummyPage(), created_by: partial, last_edited_by: personUser };
  ...
  it('should enrich datasource entities', async () => {
    ...
    fetch.mockResolvedValue(new Response(JSON.stringify(fullUser), { status: 200, headers: { 'Content-Type': 'application/json' } }));
    const resolver = new UserResolver(client);
    const dataSource = { ...buildDummyDataSource(), created_by: { object: 'user' as const, id: 'ds-user' }, last_edited_by: { object: 'user' as const, id: 'ds-user' } };
