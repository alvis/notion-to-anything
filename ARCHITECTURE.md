...
- `mapWithConcurrency()` preserves input order and limits active traversal/enrichment tasks.
- `new Notion({ concurrency })` builds a p-queue-backed `fetch` wrapper that caps in-flight SDK requests at the supplied concurrency (defaults to `Infinity`).
- The underlying `@notionhq/client` SDK is always constructed internally; the per-request `Authorization` header is injected by the wrapper from the `token` option, so consumers cannot bypass request counting, retry, or auth refresh.
...
|---|---|
| `notion.ts` | Public client, entry point for all entity retrieval and workspace search; owns the private `#metrics` instance and exposes it through a read-only `get metrics()` accessor |
...
| `request/fetch.ts` | Instrumented fetch wrapper: per-attempt bearer injection, p-queue concurrency, 429 queue-pause, one-shot 401 refresh, 403 → plain `Error` |
| `request/parse-retry-after.ts` | Retry-After parsing from headers, `additional_data`, or response body |
| `metrics/request-metrics.ts` | Request counters with pause-aware `activeMs` / `rps` snapshots + tick emitter |
| `observer.ts` | `NotionRequestObserver` interface for request lifecycle and rate-limit events |
