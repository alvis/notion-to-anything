...
**Description:** Creates a client that owns the underlying `@notionhq/client` SDK along with request counting, retry/backoff, and Notion auth lifecycle. The `auth` header is injected per request inside the built-in fetch wrapper; the SDK is constructed internally so consumers cannot bypass instrumentation.

**Parameters:**

- `options` (`NotionOptions`): configuration object (required)
- `options.token` (`string | (init: boolean) => Promise<string>`): bearer token source — see [Authentication](#authentication)
- `options.fetch` (`typeof fetch`): optional base fetch implementation (defaults to `globalThis.fetch`)
- `options.logger`: optional logger forwarded to the underlying Notion SDK client
- `options.concurrency` (`number`): positive integer in-flight cap for SDK fetches and traversal work
...
- `options.observer` (`NotionRequestObserver`): low-level request lifecycle and rate-limit events observer
...
- `client` (`Client`): the underlying `@notionhq/client` SDK instance (read-only getter) — use it for SDK operations not surfaced by the entity helpers; every call still flows through the instrumented fetch wrapper
- `metrics` (`RequestMetrics`): low-level request counters exposed via a read-only `get metrics()` accessor; the getter returns the single live `RequestMetrics` instance owned by the client (never reassigned), so every consumer observes the same evolving counters. Every attempt (initial + retries) increments `metrics.snapshot().requests`
...
The `token` option is the single source of truth for the bearer token.

- **String literal** — `token: 'secret_…'` is sent on every request as `Authorization: Bearer secret_…`.
- **Async provider** — `token: async (init) => '…'` is invoked per attempt:
  - `init === true` on the initial attempt (and on every fresh request thereafter)
  - `init === false` on the one-shot retry triggered by a `401` response
- Exactly one refresh is performed per request. If the retried attempt also returns `401`, that `401` response is surfaced to the SDK (no further refresh, no infinite loop).

#### Retry & backoff

- **`429` (rate limit):** the internal queue pauses for the parsed `Retry-After` value plus a fixed 5 s safety buffer. Sibling `429`s while the queue is paused are no-ops (first-hit-wins) and do not extend the window. Once the timer elapses, the failed attempt is re-enqueued. There is **no retry cap on 429** — the wrapper keeps retrying until a non-429 response arrives. The re-enqueue happens via `queue.add` (async re-entry) so the call stack does not grow under bursty 429s.
- **`401` (unauthorized):** the wrapper calls `token(false)` once and re-runs the attempt with the refreshed bearer. If that second attempt also returns `401`, the response is returned to the caller as-is.
- **`403` (forbidden):** the wrapper throws a plain `Error` (`new Error('Notion API forbidden (403)')`); no retry, no special class.
- All other responses (including 4xx like 404 / 5xx) flow through to the SDK untouched.

#### Errors

The wrapper does not introduce any custom error class. Failures surface as either:

- plain `Error` instances (for 403)
- the SDK's own `APIResponseError` (raised by `@notionhq/client` for terminal HTTP errors)
- whatever the configured `fetch` implementation rejects with

#### Metrics

`notion.metrics` is a read-only getter over a single `RequestMetrics` instance created once in the constructor — the client never swaps it out, so reading `notion.metrics` repeatedly always returns the same live tracker.

`metrics.recordRequest()` fires **once per attempt** — the initial attempt, the 429 retries, and the 401 refresh retry all increment `metrics.snapshot().requests`. Pause/resume hooks (`observer.onRateLimitPause`, `observer.onRateLimitResume`) wrap the queue-pause window so `metrics.snapshot().activeMs` excludes time spent waiting on rate-limit backoff.

#### Migration note

If you previously injected a pre-built SDK client:

```ts
// Before
new Notion({ client: new Client({ auth: process.env.NOTION_TOKEN }) });

// After
new Notion({ token: process.env.NOTION_TOKEN! });
```

Direct SDK access remains available via the read-only `notion.client` getter (`anything.client.users.retrieve(...)`).
